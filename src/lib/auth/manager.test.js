...
describe('auth-manager', () => {
    ...
    test('processCode', async () => {
        ...
        when(mockClient.getToken).calledWith({ code, redirect_uri: redirectUrl }).mockResolvedValue(token)
    ...
    describe('getAccessToken', () => {
        ...
        test('returns token from store', async () => {
            ...
            when(mockClient.createToken).calledWith(tokenPOJO).mockReturnValue(token)
        ...
        test('refreshes an expired token before returning it', async () => {
            ...
            mockTokenStore.get.mockResolvedValue(tokenPOJO)
            when(mockClient.createToken).calledWith(tokenPOJO).mockReturnValue(token1)
